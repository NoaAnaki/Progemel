...
const CMP_COLS = [
  ...
  { key:'foreign',   label:'חו"ל %' },
  { key:'forex',     label:'מט"ח %' },
  { key:'illiquid',  label:'לא סחיר %' },
  { key:'profit_index', label:'מדד פרופיט' },
];

function FundComparison({ funds, product }) {
  const [open, setOpen]           = useState(false);
  const [nameQuery, setNameQuery] = useState('');
  const [selected, setSelected]   = useState([]); // array of fund objects
  const [chartMetric, setChartMetric] = useState('ret_1y');

  // All unique fund names for this product
  ...
  const suggestions = useMemo(() => {
    ...
    return allNames.filter(f => f.name.includes(q)).slice(0, 8);
  }, [nameQuery, allNames]);

  function addFund(fund) {
    if (selected.length >= 10) return;
    if (selected.find(f => f.name === fund.name)) return;
  ...
  const fmtVal = (fund, key) => {
    ...
    return `${v > 0 && ['ret_month','ret_ytd','ret_1y','ret_3y','ret_5y'].includes(key) ? '+' : ''}${v.toFixed(1)}%`;
  };

  const cellColor = (fund, key) => {
    if (!selected.length || selected.length < 2) return C.dark;
    const vals = selected.map(f => f[key]).filter(v => v != null);
    if (!vals.length) return C.dark;
    const v = fund[key];
    if (v == null) return C.muted;
    const max = Math.max(...vals), min = Math.min(...vals);
    if (v === max && max !== min) return C.pos;
    if (v === min && max !== min) return C.neg;
    return C.dark;
  };
  ...
  const ChartBar = () => {
    const data = selected
      .map(f => ({ name: f.name.slice(0, 18), val: f[chartMetric] }))
      .filter(d => d.val != null)
      .sort((a, b) => b.val - a.val);
    if (!data.length) return null;
    const maxAbs = Math.max(...data.map(d => Math.abs(d.val)), 1);
    const W = 500, BAR_H = 24, GAP = 6, LABEL_W = 160, VAL_W = 50, PAD = 8;
    const H = data.length * (BAR_H + GAP) + PAD * 2;
    const chartW = W - LABEL_W - VAL_W;
    const zeroX  = LABEL_W + chartW / 2;

    return (
      <svg width="100%" viewBox={`0 0 ${W} ${H}`} style={{ display:'block', overflow:'visible', marginTop:8 }}>
        {data.map((d, i) => {
          const y     = PAD + i * (BAR_H + GAP);
          const bw    = Math.max(2, Math.abs(d.val) / maxAbs * (chartW / 2 - 4));
          const bx    = d.val >= 0 ? zeroX : zeroX - bw;
          const color = d.val >= 0 ? C.pos : C.neg;
          return (
            <g key={d.name}>
              <text x={LABEL_W - 6} y={y + BAR_H/2 + 4} textAnchor="end"
                fontSize="9.5" fill={C.mid} fontFamily="Assistant,Heebo,sans-serif">
                {d.name}
              </text>
              <rect x={LABEL_W} y={y} width={chartW} height={BAR_H} fill={C.bg} rx="3"/>
              <line x1={zeroX} y1={y} x2={zeroX} y2={y+BAR_H} stroke={C.border} strokeWidth="1"/>
              <rect x={bx} y={y+3} width={bw} height={BAR_H-6} fill={color} rx="2" opacity="0.85"/>
              <text x={LABEL_W+chartW+6} y={y+BAR_H/2+4} fontSize="9.5" fill={color}
                fontWeight="700" fontFamily="Assistant,Heebo,sans-serif">
                {fmtVal(data[i], chartMetric)}
              </text>
            </g>
          );
        })}
  ...
  return (
    <div style={{ margin:'0 0 18px', borderRadius:10, overflow:'hidden', border:`1px solid ${C.border}`, background:C.white }}>
      {/* Toggle header */}
      <button onClick={() => setOpen(o => !o)} style={{
        width:'100%', display:'flex', alignItems:'center', gap:10,
        padding:'12px 16px', background: open ? C.darkMid : C.white,
        border:'none', cursor:'pointer', fontFamily:'inherit', direction:'rtl',
        transition:'background 0.2s',
      }}>
        <span style={{ fontSize:18 }}>⚖️</span>
        <span style={{ fontSize:14, fontWeight:700, color: open ? C.white : C.dark }}>
          השוואת מסלולי השקעה
        </span>
        <span style={{ fontSize:11, color: open ? 'rgba(255,255,255,0.5)' : C.muted, fontWeight:400 }}>
          בחר עד 10 מסלולים להשוואת חשיפות ותשואות
        </span>
        <span style={{ marginRight:'auto', fontSize:16, color: open ? C.white : C.muted }}>
          {open ? '▲' : '▼'}
        </span>
        {selected.length > 0 && (
          <span style={{
            background: C.crimson, color:C.white, borderRadius:12,
            padding:'1px 9px', fontSize:11, fontWeight:700,
          }}>{selected.length} נבחרו</span>
        )}
      </button>
      ...
      {open && (
        <div style={{ padding:'14px 16px', borderTop:`1px solid ${C.border}` }}>
          ...
          <div style={{ position:'relative', marginBottom:12 }}>
            ...
            {suggestions.length > 0 && (
              <div style={{
                ...
                boxShadow:'0 8px 24px rgba(0,0,0,0.1)', zIndex:200, maxHeight:220, overflowY:'auto',
              }}>
                {suggestions.map(f => (
                  <div key={f.name} onClick={() => addFund(f)} style={{
            ...
            <div style={{ display:'flex', gap:6, flexWrap:'wrap', marginBottom:12 }}>
              {selected.map(f => (
                <span key={f.name} style={{
                  ...
                  borderRadius:16, padding:'3px 10px', fontSize:11.5,
                }}>
                  <span style={{ maxWidth:160, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{f.name}</span>
                  <button onClick={() => removeFund(f.name)} style={{
                    background:'none', border:'none', cursor:'pointer',
                    color:C.crimson, fontSize:14, padding:0, lineHeight:1,
                  }}>×</button>
                </span>
              ))}
          ...
          {selected.length > 0 && (
            <>
              <div style={{ overflowX:'auto', marginBottom:12, borderRadius:8, border:`1px solid ${C.border}` }}>
                <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                  <thead>
                    <tr style={{ background:'#2A2A2A' }}>
                      <th style={{ ...TH, textAlign:'right', color:'rgba(255,255,255,0.8)', minWidth:160 }}>קרן</th>
                      {CMP_COLS.map(c => (
                        <th key={c.key} style={{ ...TH, textAlign:'center', color:'rgba(255,255,255,0.8)' }}>
                          {c.label}
                        </th>
                      ))}
                    </tr>
                  ...
                  <tbody>
                    {selected.map((fund, ri) => (
                      <tr key={fund.name} style={{
                        ...
                        <td style={{ ...TD, fontWeight:600, color:C.dark, maxWidth:200 }}>
                          <div style={{ overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }} title={fund.name}>
                            {fund.name}
                          </div>
                        </td>
