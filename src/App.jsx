    ...
    <div style={{ position:'relative' }}>
          ...
          return (
            <g key={p.key}>
              ...
              {avg != null && (() => {
                ...
                return (
                  <circle cx={cx} cy={avgY} r="5" fill={C.crimson} opacity="0.85"
                    style={{ cursor:'pointer' }}
                    onMouseEnter={() => setHoveredAvg({ label:p.label, val:avg, cx, cy:avgY })}
      ...
      {hoveredAvg && (
        <div style={{
          ...
          top:`calc(${(hoveredAvg.cy / (H + 6)) * 100}% - 44px)`,
...
// ── Load Profit Index from Google Sheet ───────────────────────────────────────
// Sheet format: row1 = "provider", name1, name2, ...
//               row2 = "score",    val1,  val2,  ...
const PROFIT_SHEET_CSV = "https://docs.google.com/spreadsheets/d/e/2PACX-1vTBaP0TxMSmULSqhQb3ORgKHEwb8MB2jNuSOFn-3rsUofFZNTsj2RuDUkTihBGTdPjvSaw1e5TLfUjm/pub?output=csv";

function useProftIndex() {
  const [indexMap, setIndexMap] = useState({});
  useEffect(() => {
    fetch(PROFIT_SHEET_CSV)
      .then(r => r.text())
      .then(csv => {
        ...
        const map = {};
        // names[0] = "provider", scores[0] = "score" — skip index 0
        for (let i = 1; i < names.length; i++) {
          const val = parseFloat(scores[i]);
          if (names[i] && !isNaN(val)) map[names[i]] = val;
        }
        setIndexMap(map);
      })
      .catch(() => {});
...
export default function App() {
  ...
  const profitIndex = useProftIndex();

  const rawFunds = useMemo(() => getAllFunds(product), [product]);
  // Enrich funds with profit_index from sheet
  const funds = useMemo(() =>
    rawFunds.map(f => ({
      ...f,
      profit_index: profitIndex[f.name] ?? f.profit_index ?? null,
    })),
  ...
  return (
    <div style={{ minHeight:'100vh', background:C.bg, fontFamily:"'Assistant','Heebo',Arial,sans-serif", direction:'rtl' }}>
      ...
      <nav style={{
        ...
        <div style={{ color:'rgba(255,255,255,0.25)', fontSize:11, letterSpacing:'0.04em' }}>
          ProGemel
        </div>
      </nav>
